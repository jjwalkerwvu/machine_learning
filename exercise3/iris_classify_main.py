...
with open('bezdekIris.data', 'rb') as f:
	reader = csv.reader(f)
 	iris_dimensions = list(reader)

# figure out how many rows of data we have in the list:
# apparently there are 2 blank rows in the data file, hence why we subtract 2:
data_len=len(iris_dimensions)-2	


# I guess I have to define these lists first?
sepal_length=np.zeros((data_len,1))
sepal_width=np.zeros((data_len,1))
petal_length=np.zeros((data_len,1))
petal_width=np.zeros((data_len,1))
iris_class=np.zeros((data_len,1))

# fill the arrays with all the relevant data
for i in range(0,data_len):
# get the sepal length, in a floating point value
	temp=iris_dimensions[i]
	sepal_length[i]=np.float(temp[0])
	sepal_width[i]=np.float(temp[1])
	petal_length[i]=np.float(temp[2])
	petal_width[i]=np.float(temp[3])
	# use logic here to fill in the correct class;
	# 1 = 'Iris-virginica'
	# 2 = 'Iris-versicolor'
	# 3 = 'Iris-setosa'
	iris_class[i]=3*(temp[4]=='Iris-virginica')+2*(temp[4]=='Iris-versicolor')+1*(temp[4]=='Iris-setosa')
	# should probably be some better logic to automatically pick out the 
	# each individual label, and then determine the number of labels from that
	num_labels=3;

# can just make iris_class an integer array?
#iris_class=int(iris_class)
#####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#####
